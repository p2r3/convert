...
function createPortal(blue = true) {
  ...
  const portal = new THREE.Mesh(portalGeometry, new THREE.MeshBasicMaterial({ color }));
...
class sppdHandler implements FormatHandler {
  ...
  private entityObjects: any[] = Array.from({ length: 2048 });
  ...
  addVoxelsAlongRay(start: Vector, end: Vector, voxels: Map<string, Vector>) {
    ...
    let tMaxX =
      stepX !== 0
        ? (stepX > 0 ? Math.floor(gx0) + 1 - gx0 : gx0 - Math.floor(gx0)) * tDeltaX
        : Infinity;
    let tMaxY =
      stepY !== 0
        ? (stepY > 0 ? Math.floor(gy0) + 1 - gy0 : gy0 - Math.floor(gy0)) * tDeltaY
        : Infinity;
    let tMaxZ =
      stepZ !== 0
        ? (stepZ > 0 ? Math.floor(gz0) + 1 - gz0 : gz0 - Math.floor(gz0)) * tDeltaZ
        : Infinity;
  ...
  collectPoints(demo: Demo, voxels: Map<string, Vector>, portalVoxels: Map<string, Vector>) {
    ...
    if (this.voxelGridOffset === null) {
      ...
      if (!door) this.voxelGridOffset = new Vector();
        ...
        this.voxelGridOffset = doorPosition.Add(doorForward.Scale(-16)).map((c) => c % 64);
      }
    }
      ...
      for (const x of [mins.x, maxs.x]) {
        for (const y of [mins.y, maxs.y]) {
          for (const z of [mins.z, maxs.z]) {
            const point = pos.Add(forward.Scale(x)).Add(right.Scale(-y)).Add(up.Scale(z));
